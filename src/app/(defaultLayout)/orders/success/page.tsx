"use client";

import { useEffect } from "react";
import { useAppDispatch } from "@/redux/hooks";
import { clearCart } from "@/redux/features/cartSlice";
import { Button } from "@/components/ui/button";
import { CheckCircle2, Gift, ArrowRight, Home } from "lucide-react";
import Link from "next/link";
import { useSearchParams } from "next/navigation";
import { motion } from "framer-motion";

export default function OrderSuccessPage() {
  const dispatch = useAppDispatch();
  const searchParams = useSearchParams();
  const transactionId = searchParams.get("transactionId");

  useEffect(() => {
    // Clear cart on successful order
    dispatch(clearCart());
  }, [dispatch]);

  return (
    <div className="min-h-screen bg-neutral-50 flex items-center justify-center p-4 pt-24">
      <motion.div 
        initial={{ opacity: 0, scale: 0.9, y: 20 }}
        animate={{ opacity: 1, scale: 1, y: 0 }}
        className="max-w-xl w-full bg-white rounded-[3rem] p-12 text-center shadow-2xl border border-neutral-100 space-y-8"
      >
        <div className="relative">
           <div className="absolute inset-0 bg-green-500/10 rounded-full blur-3xl animate-pulse"></div>
           <div className="relative w-24 h-24 bg-green-500 rounded-3xl flex items-center justify-center mx-auto text-white shadow-xl rotate-12">
              <CheckCircle2 className="w-12 h-12 -rotate-12" />
           </div>
        </div>

        <div className="space-y-4">
          <h1 className="text-4xl font-extrabold text-neutral-900 tracking-tight">Order Successful!</h1>
          <p className="text-neutral-500 text-lg">
            Thank you for your purchase. Your magic gift is now being prepared.
          </p>
          <div className="inline-block px-4 py-2 bg-neutral-50 rounded-full border border-neutral-100">
             <p className="text-xs font-bold text-neutral-400 uppercase tracking-widest">
               Transaction ID: <span className="text-neutral-900">{transactionId}</span>
             </p>
          </div>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Button asChild className="h-14 bg-neutral-900 hover:bg-neutral-800 text-white rounded-2xl font-bold text-lg gap-2">
            <Link href="/profile/orders">
              View Order History
              <ArrowRight className="w-5 h-5" />
            </Link>
          </Button>
          <Button asChild variant="outline" className="h-14 rounded-2xl font-bold text-lg gap-2">
            <Link href="/">
              <Home className="w-5 h-5" />
              Back to Home
            </Link>
          </Button>
        </div>

        <div className="pt-8 flex items-center justify-center gap-2 text-primary font-bold">
           <Gift className="w-5 h-5" />
           <span>GiftAI Magic</span>
        </div>
      </motion.div>
    </div>
  );
}
